...
const components: Partial<PortableTextReactComponents> = {
  types: {
    image: ({value}): React.ReactElement => {
      return (
        <Image src={SanityImgUrl.getUrlBuilder(value).url()} alt="project-img" width={640} height={320} className="rounded-xl border-2 my-2 self-center"/>
      );
    }
  }
}
...
export default async function ProjectDetailPage({params}: { params: { slug: string } }) {
  ...
  return (
    <main className="flex min-h-screen flex-col items-center px-4 py-8 md:px-8 lg:px-16">
      ...
      <Image src={project.imgUrl} className="rounded-xl border-2 my-4 scale-90" alt="project-img" width={640} height={320}/>
